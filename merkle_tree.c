...
#include <string.h>
#include <assert.h>
#include "../xmss_integration/xmss_submodule/sha256.h"
...
void cbmt_build_merkle_root(const hash_t* leaves, size_t leaves_count, const hash_algo_t* algo, hash_t result) {
    ...
    while (queue->count > 1) {
        // Copy to contiguous buffer for hash function
        memcpy(buffer, queue->data[0], HASH_SIZE);
        memcpy(buffer + HASH_SIZE, queue->data[1], HASH_SIZE);
        
        // Remove first two elements
        memmove(queue->data, queue->data + 2, (queue->count - 2) * sizeof(hash_t));
        queue->count -= 2;
        
        algo->hash_func(buffer, buffer + HASH_SIZE * 2, merged);
...
merkle_tree_t* cbmt_build_merkle_tree(const hash_t* leaves, size_t leaves_count, const hash_algo_t* algo) {
    ...
    for (size_t i = leaves_count - 1; i > 0; i--) {
        ...
        if (left_child < total_nodes && right_child < total_nodes) {
            // Pass range covering both nodes: [left_child, right_child + HASH_SIZE)
            algo->hash_func(tree->nodes[left_child], tree->nodes[right_child] + HASH_SIZE, tree->nodes[i - 1]);
        }
    }
